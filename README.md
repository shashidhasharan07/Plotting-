...
def decode_struct(icd, payload, offset, prefix, context):
    ...
    values = struct.unpack_from(icd["fmt"], payload, offset)
    offset += size
    ...
    return decoded, labels, offset


def decode_dynamic(icd, payload, offset, prefix, context):
    ...
    if "count_fmt" in icd:
        ...
        if offset + csize > len(payload):
            return decoded, labels, len(payload)

        n = struct.unpack_from(icd["count_fmt"], payload, offset)[0]
        offset += csize
    ...
    for i in range(n):
        ...
        d, l, offset = decode_block(
            icd["element"], payload, offset, sub_prefix, context
        )
...
def decode_mixed(icd, payload, offset, prefix, context):
    ...
    for block in icd["blocks"]:
        ...
        d, l, offset = decode_block(
            block, payload, offset, prefix, context
        )
...
def decode_block(icd, payload, offset, prefix, context):

    t = icd["type"]

    if t == "struct":
        return decode_struct(icd, payload, offset, prefix, context)

    if t == "dynamic_array":
        return decode_dynamic(icd, payload, offset, prefix, context)

    if t == "mixed":
        return decode_mixed(icd, payload, offset, prefix, context)
...
class PlotWindow(QtWidgets.QWidget):
    ...
    def update_plot(self):
        ...
        for i, (opcode_box, combo, plot, curve, value_label, packet_label) in enumerate(zip(
            self.opcode_boxes,
            self.param_boxes,
            self.plots,
            self.curves,
            ...
            curve.setData(
                fx[-MAX_RENDER_POINTS:],
                fy[-MAX_RENDER_POINTS:],
